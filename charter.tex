...
La aplicación permite seleccionar filtros de procesamiento, previamente generados, de un banco de filtros almacenado en memoria, realizar la re-configuración dinámica y evaluar el funcionamiento del banco seleccionado. Como componente principal, el dispositivo a desarrollar cuenta con un System on Chip (SoC) o Multi Processor System on Chip (MPSoC).
...
El entorno de pruebas se debe implementar haciendo uso de un kit de desarrollo que contenga un SoC o MPSoC de serie 7 o superior de la marca Xilinx. Por lo que, se debe establecer los mecanismos de re-configuración mediante el entorno de desarrollo Vivado.
...
\begin{itemize}
	\item Cliente: profesional altamente calificado en la industria aeroespacial y defensa.
	\item Orientador: \supname, no va a estar disponible hasta Octubre de 2021 y probablemente luego de esa fecha esté viviendo en Europa, por lo que hay que tener en cuenta ese hecho cuando se decida contactarlo.
...
El responsable del proyecto dispone de una placa ZedBoard para la realización del desarrollo, para que el banco de pruebas sea implementado en una tecnología más moderna, se debe evaluar si INVAP dispone de un hardware de desarrollo que contenga un MPSoC.
